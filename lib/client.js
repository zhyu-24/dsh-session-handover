		...
		*/
		...
		function apply(ctx) {
			...
			function DerivePanel() {
				...
				const confirm = () => {
					const picked = s.candidates.filter((c) => s.checked[c.label]);
					const chosen = picked.map((c) => c.label);
					const scope = picked.map((c) => String(c.description || "").trim()).filter(Boolean).join("；");
				...
				if (s.phase === "done") {
					...
					if (d.md) children.push(react.createElement("pre", {
						key: "done-md",
						style: {
							maxHeight: "44vh",
							overflowY: "auto",
				...
				if (s.phase === "done") children.push(react.createElement("div", {
					key: "done-actions",
					style: {
						display: "flex",
						gap: 8,
				...
				}, "复制全文"), react.createElement("button", {
					onClick: close,
					style: {
						padding: "4px 12px",
						borderRadius: 6,
