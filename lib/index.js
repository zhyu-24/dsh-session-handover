...
function unescapeJsonString(s) {
	return String(s).replace(/\\(u[0-9a-fA-F]{4}|[\\"ntrbf/])/g, (_m, e) => {
		if (e === "n") return "\n";
		if (e === "t") return "	";
		if (e === "r") return "\r";
		if (e === "b") return "\b";
...
function parseFinalize(raw) {
	...
	if (m) {
		...
		if (md) {
			...
			return {
				slug: slugM ? String(slugM[1]).trim() : "",
				md
			};
		}
	}
			...
			handler: async (req, res) => {
				try {
					...
					for (let attempt = 0; attempt < 2 && !md; attempt++) {
						attempts++;
						const raw = await runModel(ctx, system, user, 12e3, attempt === 0 ? .2 : .6);
						rawLogs.push({
							len: raw.length,
					...
					if (!md) {
						...
						if (dbgCwd) try {
							...
							const dbgPolicy = dbgPolicySvc ? dbgPolicySvc.resolve(dbgLive ? { session: dbgLive } : {}) : void 0;
							...
							await dbgFs.writeText(dbgTarget, payload, void 0, void 0, dbgPolicy);
							debugFile = "dsh-handover-finalize-debug.json";
						} catch {}
						writeJson(res, 500, { error: "模型连续两次未生成有效交接文档，请重试" + (debugFile ? "（诊断已写入工作区 " + debugFile + "）" : "") });
						return;
					}
